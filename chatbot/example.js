...
function myFunction(){
alert('changes'+voiceSelecter.value)
localStorage.setItem('voice',voiceSelecter.value)
...
function playsyncronized(text='no words'){
	console.log("sync called",text)
	...
	//	var text = document.getElementById('texttospeakinput').value;
		...
		var voice = speechSynthesis.getVoices().filter(function(voice){
				...
				console.log(' voiceSelecter.value;'+ voiceSelecter.value,"local voice",localStorage.getItem('voice'))
				var voicelocal=localStorage.getItem('voice')
				return voice.name == voicelocal
